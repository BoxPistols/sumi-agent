...
export const REGEX_PATTERNS: RegexPattern[] = [
  ...
  },
  {
    ...
    category: 'address',
    regex:
      /(?:〒\s?\d{3}[-ー]\d{4}|(?<!\d)(?<![-ー])\d{3}[-ー]\d{4}(?![-ー]\d)(?!\d))/g,
  },
  {
    id: 'birthday',
    label: '年月日',
  ...
  for (const p of REGEX_PATTERNS) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      if (p.id === 'phone' || p.id === 'postal' || p.id === 'mynumber') {
        ...
        const hasPIILabel =
          /(?:電話|TEL|tel|Tel|携帯|FAX|fax|連絡先|〒|郵便)\s*[：:・]?\s*$/.test(
      ...
      if (p.id === 'birthday') {
        ...
        const isBirthdayLabel =
          /(?:生年月日|誕生日|生まれ|DOB|Date of Birth)\s*[：:・]?\s*$/i.test(before30)
...
const NON_NAME_WORDS = new Set([
  "株式会社","有限会社","合同会社","一般社団","特定非営利","事業部","開発部","営業部","総務部","人事部","経理部","企画部","技術部","管理部","製造部","品質管理","情報システム","エンジニア","マネージャー","ディレクター","プロジェクト","アシスタント","コンサルタント","デザイナー","マーケター","プログラマー","アナリスト","インターン","フロントエンド","バックエンド","フルスタック","テックリード","アドバイザー","クリエーター","プランナー","リサーチャー","スペシャリスト","コーディネーター","マーケティング","ブランディング","コンサルティング","エグゼクティブ","プレジデント","チーフ","シニア","ジュニア","リード","ヘッド","美容師","薬剤師","看護師","弁護士","税理士","会計士","司法書士","行政書士","社労士","建築士","技術者","研究者","教授","講師","助手","学生","院生","新卒","中途","派遣","契約","正社員","パート","アルバイト","代表取締役","取締役","監査役","執行役員","副社長","専務","常務","部長代理","次長","主幹","係長補佐","班長","組長","チームリーダー","グループリーダー","セクションリーダー","技術顧問","顧問","相談役","参与","特別顧問","社外取締役","非常勤","嘱託","名誉会長","名誉顧問","最高顧問","経営顧問","法律顧問","技術担当","事業担当","統括責任者","統括部長","本部長","副本部長","支社長","支店長","工場長","所長","室長","センター長","部門長","課長代理","主任技師","主任研究員","技術主任","開発主任","プロダクトマネージャー","スクラムマスター","テクニカルリード","アーキテクト","データサイエンティスト","機械学習エンジニア","インフラエンジニア","セキュリティエンジニア","品質保証","テスター","カスタマーサクセス","アカウントマネージャー","事業開発","経営企画","広報担当","人事担当","法務担当","財務担当","経理担当","総務担当","情報管理","会長","社長","部長","課長","係長","主任","店長","院長","園長","館長","署長","局長","議長","委員長","理事長","学長","校長","教頭","学部長","研究室長",
])
...
export function detectJapaneseNames(text: string): Detection[] {
  ...
  while ((lm = lre.exec(text)) !== null) {
    ...
    if (!found) {
      const nameGuess = afterLabel.match(
        /^([\u4e00-\u9fff]{2,4}[\s\u3000]?[\u4e00-\u9fff]{1,4})/
      )
