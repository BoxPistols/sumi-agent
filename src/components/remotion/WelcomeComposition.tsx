...
const Subtitle: React.FC<{ children: React.ReactNode; delay?: number }> = ({
  children,
  delay = 0,
}) => (
...
const ExampleRow: React.FC<{ label: string; examples: string[]; delay: number; color: string }> = ({
  label,
  examples,
  delay,
  color,
  ...
  return (
    ...
    >
      <span style={{ fontSize: 11, fontWeight: 700, color, minWidth: 90, textAlign: 'right' }}>
        {label}
      </span>
...
const Slide2Japanese: React.FC = () => (
  <SlideBase gradient="linear-gradient(150deg, #0f172a 0%, #172033 50%, #0f172a 100%)">
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 28 }}>
      ...
      <ExampleRow
        label="和暦/西暦"
        examples={['令和6年', '2024年', 'R6']}
        delay={34}
        color={AMBER}
      />
      <ExampleRow
        label="全角/半角"
        examples={['090-1234-5678', '０９０−１２３４−５６７８']}
        delay={42}
        color={ACCENT}
      />
    </div>
    <Body delay={52}>英語向けツールでは対応できない。日本語に特化した検出が必要です。</Body>
  </SlideBase>
)
...
const Slide3Detection: React.FC = () => (
  <SlideBase gradient="linear-gradient(140deg, #0f172a 0%, #1a1033 40%, #151030 100%)">
    ...
    <Body delay={16}>単一の手法では精度が不十分。4つの層が互いの弱点を補完します。</Body>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 24 }}>
      <LayerCard
        num={1}
        label="正規表現"
        desc="メール・電話・住所・生年月日などのパターンマッチ"
        delay={28}
        color={BLUE}
      />
      <LayerCard
      ...
      <LayerCard
        num={4}
        label="AI補完"
        desc="Claude / GPT / Gemini による追加検出"
        delay={52}
        color={ACCENT}
      />
    </div>
  ...
  <SlideBase>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 28 }}>
      <FlowStep
        num={1}
        label="ファイルをアップロード"
        sub="PDF, Word, Excel, CSV など多数の形式に対応"
        delay={18}
      />
      ...
      <FlowStep
        num={3}
        label="確認・調整"
        sub="検出結果を個別にON/OFF。Diff表示で前後比較"
        delay={38}
      />
      <FlowStep
        num={4}
        label="エクスポート"
        sub="テキスト、PDF、Word、Excelなど6形式で出力"
        delay={48}
      />
    </div>
    <FadeSlide delay={60}>
