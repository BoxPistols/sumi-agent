...
RedactPro -- 日本語履歴書・職務経歴書のPII自動検出・マスキングツール。Next.js 15 (App Router) + React 19 + TypeScript。Lite版（シンプル）とPro版（全機能）をランタイムstate切替で提供。
...
| コマンド          | 用途                                                             |
| ----------------- | ---------------------------------------------------------------- |
| `pnpm dev`        | 開発サーバー（portless経由、`http://redact-pro.localhost:1355`） |
| `pnpm build`      | プロダクションビルド（distDir: `.next-build`）                   |
| `pnpm test`       | Vitest テスト実行（272テスト）                                   |
...
- `C` -- ハードコードカラー定数（accent, purple, green等）
- `T` -- CSS Custom Properties参照（`var(--rp-text)` 等）。ダーク/ライト切替で値が変わる
